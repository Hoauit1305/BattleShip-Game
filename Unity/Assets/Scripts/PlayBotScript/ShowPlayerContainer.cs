...
public class ShowPlayerContainer : MonoBehaviour
{
    ...
    IEnumerator LoadAndShowPlayerShips()
    {
        ...
        string apiURL = "https://battleship-game-production-1176.up.railway.app/api/gameplay/showship";
        ...
        if (webRequest.result == UnityWebRequest.Result.Success)
        {
            ...
            {
                // Parse JSON response
                PlayerShipData[] playerShips = JsonHelper.FromJson<PlayerShipData>(responseText);
                ...
                if (playerShips != null && playerShips.Length > 0)
                {
                    ...
                    // Hiển thị từng tàu
                    foreach (PlayerShipData ship in playerShips)
                    {
                        ShowPlayerShip(ship);
                    }
                }
    ...
    void ShowPlayerShip(PlayerShipData shipData)
    {
    ...
    void ConfigurePlayerShipVisual(GameObject shipObject, PlayerShipData shipData, bool isVertical)
    {
    ...
    // Tạo POST request
    UnityWebRequest CreatePostRequest(string url, ShowShipRequest requestData)
    {
...
[System.Serializable]
public class ShowShipRequest
{
    public string gameId;

    public ShowShipRequest(string gameId)
    {
        this.gameId = gameId;
    }
}
