...
public class PlayBot : MonoBehaviour
{
    ...
    IEnumerator SetIDAndStartMatch()
    {
        ...
        UnityWebRequest request = new UnityWebRequest("https://battleship-game-production-1176.up.railway.app/api/gameplay/create-gameid", "POST"); // ✅ ĐÃ SỬA
    ...
    IEnumerator PlaceShipsForBot(int gameId, string token)
    {
        ...
        UnityWebRequest request = new UnityWebRequest("https://battleship-game-production-1176.up.railway.app/api/gameplay/place-ship/bot", "POST");
        request.uploadHandler = new UploadHandlerRaw(bodyRaw);
        request.downloadHandler = new DownloadHandlerBuffer();
        request.SetRequestHeader("Authorization", "Bearer " + token);
        request.SetRequestHeader("Content-Type", "application/json");
